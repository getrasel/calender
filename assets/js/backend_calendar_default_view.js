...
(function (exports) {
    ...
    function bindEventHandlers() {
        ...
        $calendarPage.on("click", ".delete-popover", function () {
            ...
            if (lastFocusedEventData.data.workingPlanException) {
                ...
                var successCallback = function () {
                    Backend.displayNotification(
                        EALang.working_plan_exception_deleted
                    );
                ...
                BackendCalendarApi.deleteWorkingPlanException(
                    date,
                    providerId,
                    successCallback
                );
        ...
        $("#select-filter-item").on("change", function () {
            ...
            if (
                ...
                if (provider && provider.timezone) {
                    $(".provider-timezone").text(
                        GlobalVariables.timezones[provider.timezone]
                    );
                }
    ...
    function calendarEventResize(event, delta, revertFunc) {
        ...
        if (Boolean(Number(event.data.is_unavailable)) === false) {
            ...
            BackendCalendarApi.saveUnavailable(
                unavailable,
                successCallback,
                null
            );
        ...
        return $.post(url, data)
            .done(function (response) {
                ...
                if (
                    ...
                    var firstWeekdayNumber = GeneralFunctions.getWeekDayId(
                        GlobalVariables.firstWeekday
                    );
                    var sortedWorkingPlan = GeneralFunctions.sortWeekDictionary(
                        workingPlan,
                        firstWeekdayNumber
                    );
                    ...
                    switch (calendarView.name) {
                        case "agendaDay":
                            weekdayNumber = parseInt(
                                calendarView.start.format("d")
                            );
                            weekdayName =
                                GeneralFunctions.getWeekdayName(weekdayNumber);
                            weekdayDate = calendarView.start
                                .clone()
                                .format("YYYY-MM-DD");
                            ...
                            if (
                                ...
                                workingPlanExceptions[weekdayDate]
                            ) {
                                sortedWorkingPlan[weekdayName] =
                                    workingPlanExceptions[weekdayDate];
                                ...
                                workingPlanExceptionEvent = {
                                    ...
                                    editable: false,
                                    className:
                                        "fc-working-plan-exception fc-custom",
                                    data: {
                                        date: weekdayDate,
                                        workingPlanException:
                                            workingPlanExceptions[weekdayDate],
                            ...
                            viewStart = moment(
                                calendarView.start.format("YYYY-MM-DD") +
                                    " 00:00:00"
                            );
                            startHour =
                                sortedWorkingPlan[weekdayName].start.split(":");
                            ...
                            viewEnd = moment(
                                calendarView.end.format("YYYY-MM-DD") +
                                    " 00:00:00"
                            );
                            endHour =
                                sortedWorkingPlan[weekdayName].end.split(":");
                            ...
                            sortedWorkingPlan[weekdayName].breaks.forEach(
                                function (breakPeriod) {
                                    var breakStartString =
                                        breakPeriod.start.split(":");
                        ...
                        case "agendaWeek":
                            ...
                            while (calendarDate < calendarView.end) {
                                ...
                                weekdayName =
                                    GeneralFunctions.getWeekdayName(
                                        weekdayNumber
                                    );
    ...
    exports.initialize = function () {
        ...
        $("#calendar").fullCalendar({
            ...
            select: function (start, end) {
                ...
                if (
                    ...
                    service = GlobalVariables.availableServices.find(function (
                        service
                    ) {
                        return (
                            Number(service.id) ===
                            Number($("#select-filter-item").val())
                        );
                    });
                    ...
                    if (!$("#select-service").val()) {
                        $("#select-service option:first").prop(
                            "selected",
                            true
                        );
                    }
                    ...
                    if (!$("#select-provider").val()) {
                        $("#select-provider option:first").prop(
                            "selected",
                            true
                        );
                    }
        ...
        if (GlobalVariables.availableServices.length > 0) {
            $("<optgroup/>", {
                ...
                type: "services-group",
                html: GlobalVariables.availableServices.map(function (
                    availableService
                ) {
                    return $("<option/>", {
        ...
        if (GlobalVariables.availableProviders.length > 0) {
            $("<optgroup/>", {
                ...
                html: GlobalVariables.availableProviders.map(function (
                    availableProvider
                ) {
                    var hasGoogleSync =
                        availableProvider.settings.google_sync === "1"
                            ? "true"
                            : "false";
        ...
        if (GlobalVariables.user.role_slug === Backend.DB_SLUG_SECRETARY) {
            ...
            $('#select-filter-item option[type="provider"]').each(function (
                ...
                var provider = GlobalVariables.secretaryProviders.find(
                    function (secretaryProviderId) {
                        return (
                            Number($(option).val()) ===
                            Number(secretaryProviderId)
                        );
                    }
                );
