...
(function (exports) {
    ...
    exports.updateConfirmFrame = function () {
        ...
        data.customer = {
            ...
            timezone: $("#select-timezone-input").val(),
        };
        ...
        data.appointment = {
            start_datetime:
                $("#select-date").datepicker("getDate").toString("yyyy-MM-dd") +
                " " +
                Date.parse($(".selected-hour").data("value") || "").toString(
                    "HH:mm"
                ) +
                ":00",
    ...
    function calculateEndDatetime() {
        ...
        if (service.duration && startDatetime) {
            endDatetime = startDatetime.add({
                minutes: parseInt(service.duration),
            });
        } else {
    ...
    function applyAppointmentData(appointment, provider, customer) {
        try {
            ...
            if (customer.timezone) {
                $("#select-timezone-input").val(customer.timezone);
            }
    ...
    function updateServiceDescription(serviceId) {
        ...
        if (service.description) {
            ...
            $("<span/>", {
                html: GeneralFunctions.escapeHtml(
                    service.description
                ).replaceAll("\n", "<br/>"),
            }).appendTo($serviceDescription);
        }
