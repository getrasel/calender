...
(function () {
    ...
    ProvidersHelper.prototype.bindEventHandlers = function () {
        ...
        $("#providers").on(
            ...
            function () {
                ...
                $("#providers .record-details")
                    .find("input, select, textarea")
                    .prop("disabled", false);
                $("#provider-password, #provider-password-confirm").addClass(
                    "required"
                );
                $("#providers")
                    .find(
        ...
        $("#providers").on("click", "#edit-provider", function () {
            ...
            $("#providers .record-details")
                .find("input, select, textarea")
                .prop("disabled", false);
            $("#provider-password, #provider-password-confirm").removeClass(
                "required"
            );
            $("#provider-services input:checkbox").prop("disabled", false);
            $("#providers")
            ...
            function () {
                var provider = {
                    ...
                    settings: {
                        ...
                        notifications: $("#provider-notifications").prop(
                            "checked"
                        ),
    ...
    ProvidersHelper.prototype.save = function (provider) {
        var url =
            ...
            "/index.php/backend_api/ajax_save_provider";
        var data = {
    ...
    ProvidersHelper.prototype.delete = function (id) {
        var url =
            GlobalVariables.baseUrl +
            "/index.php/backend_api/ajax_delete_provider";
        var data = {
            csrfToken: GlobalVariables.csrfToken,
            provider_id: id,
        };
    ...
    ProvidersHelper.prototype.validate = function () {
        ...
        try {
            ...
            if (
                ...
                $("#provider-password-confirm").val()
            ) {
                $("#provider-password, #provider-password-confirm")
                    .closest(".form-group")
                    .addClass("has-error");
            ...
            if (
                ...
                $("#provider-password, #provider-password-confirm")
                    ...
                    .addClass("has-error");
                throw new Error(
                    EALang.password_length_notice.replace(
                        "$number",
                        BackendUsers.MIN_PASSWORD_LENGTH
                    )
                );
    ...
    ProvidersHelper.prototype.resetForm = function () {
        ...
        $("#providers .record-details #provider-timezone").val(
            "America/Los_Angeles"
        );
        $(
            "#providers .add-break, .add-working-plan-exception, #reset-working-plan"
        ).prop("disabled", true);
        ...
        $(".working-plan-exceptions")
            .find(
                ".edit-working-plan-exception, .delete-working-plan-exception"
            )
            .prop("disabled", true);
    ...
    ProvidersHelper.prototype.display = function (provider) {
        ...
        $("#provider-notifications").prop(
            "checked",
            Boolean(Number(provider.settings.notifications))
        );
        ...
        provider.services.forEach(function (providerServiceId) {
            var $checkbox = $(
                '#provider-services input[data-id="' + providerServiceId + '"]'
            );
        ...
        $(".breaks").find(".edit-break, .delete-break").prop("disabled", true);
        $("#providers .working-plan-exceptions tbody").empty();
        var workingPlanExceptions = $.parseJSON(
    ...
    ProvidersHelper.prototype.getFilterHtml = function (provider) {
        ...
        info = provider.mobile_number
            ? info + ", " + provider.mobile_number
            : info;

        info = provider.phone_number
            ? info + ", " + provider.phone_number
            : info;
    ...
    ProvidersHelper.prototype.select = function (id, display) {
        ...
        $('#filter-providers .provider-row[data-id="' + id + '"]').addClass(
            "selected"
        );
