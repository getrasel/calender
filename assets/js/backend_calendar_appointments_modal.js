...
(function (exports) {
    ...
    function bindEventHandlers() {
        ...
        $("#insert-appointment").on("click", function () {
            ...
            if (
                ...
                var providers = GlobalVariables.availableProviders.filter(
                    function (provider) {
                ...
                $dialog
                    .find("#select-service option:first")
                    .prop("selected", true)
                    .trigger("change");
            }
         ...
         */
        $("#manage-appointment").on(
            "click",
            "#existing-customers-list div",
            function () {
                var customerId = $(this).attr("data-id");

                var customer = GlobalVariables.customers.find(function (
                    customer
                ) {
                ...
                $.post(url, data)
                    .done(function (response) {
                        ...
                        response.forEach(function (customer) {
                            ...
                            var result = GlobalVariables.customers.filter(
                                function (globalVariablesCustomer) {
                                    return (
                                        Number(globalVariablesCustomer.id) ===
                                        Number(customer.id)
                                    );
                                }
                            );
                    ...
                    .fail(function (jqXHR, textStatus, errorThrown) {
                        ...
                        GlobalVariables.customers.forEach(function (
                            customer,
                            index
                        ) {
                            if (
                                ...
                                customer.last_name
                                    .toLowerCase()
                                    .indexOf(key) !== -1 ||
                                customer.email.toLowerCase().indexOf(key) !==
                                    -1 ||
                                customer.phone_number
                                    .toLowerCase()
                                    .indexOf(key) !== -1 ||
                                customer.address.toLowerCase().indexOf(key) !==
                                    -1 ||
                                customer.city.toLowerCase().indexOf(key) !==
                                    -1 ||
                                customer.zip_code.toLowerCase().indexOf(key) !==
                                    -1 ||
        ...
        $("#select-service").on("change", function () {
            ...
            GlobalVariables.availableProviders.forEach(function (provider) {
                provider.services.forEach(function (providerServiceId) {
                    if (
                        GlobalVariables.user.role_slug ===
                            Backend.DB_SLUG_PROVIDER &&
    ...
    exports.resetAppointmentDialog = function () {
        ...
        $dialog
            .find("#select-service")
            .val($dialog.find("#select-service").eq(0).attr("value"));
        ...
        var service = GlobalVariables.availableServices.forEach(function (
            service
        ) {
    ...
    function validateAppointmentForm() {
        ...
        try {
            ...
            if (start > end) {
                $dialog
                    .find("#start-datetime, #end-datetime")
                    .closest(".form-group")
                    .addClass("has-error");
