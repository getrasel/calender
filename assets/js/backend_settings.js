    ...
    exports.initialize = function (defaultEventHandlers) {
        ...
        GlobalVariables.settings.system.forEach(function (setting) {
            ...
            if (setting.name === "display_terms_and_conditions") {
                $("#display-terms-and-conditions").prop(
                    "checked",
                    Boolean(Number(setting.value))
                );
            }
            ...
            if (setting.name === "terms_and_conditions_content") {
                $("#terms-and-conditions-content").trumbowyg(
                    "html",
                    setting.value
                );
            }
        ...
        $("#calendar-view").val(
            GlobalVariables.settings.user.settings.calendar_view
        );
        $("#user-notifications").prop(
            "checked",
            Boolean(
                Number(GlobalVariables.settings.user.settings.notifications)
            )
        );
    ...
    function bindEventHandlers() {
        ...
        $("#apply-global-working-plan").on("click", function () {
            var buttons = [
                ...
                },
                {
                    ...
                    click: function () {
                        var url =
                            GlobalVariables.baseUrl +
                            "/index.php/backend_api/ajax_apply_global_working_plan";
                        ...
                        $.post(url, data)
                            .done(function () {
                                Backend.displayNotification(
                                    EALang.working_plans_got_updated
                                );
                            })
                            .always(function () {
                                $("#message-box").dialog("close");
            ...
            GeneralFunctions.displayMessageBox(
                EALang.working_plan,
                EALang.overwrite_existing_working_plans,
                buttons
            );
        });
    }
